...
generator = pipeline("text-generation", model="NousResearch/Llama-2-7b-chat-hf", device_map="auto")
...
def prepare_prompt(data, person_id):
    ...
    prompt = (
        ...
        f"{movement_phrase}, "
        f"{low_breathing} 'Low Breathing' event, and {no_move} 'No Move' event. "
        f"Generate a paragraph summarizing this single person sleep pattern."
    )
    return prompt
...
pd.DataFrame(results).to_csv("generated_summaries_llama.csv", index=False)
print("\nAll summaries saved to 'generated_summaries_llama.csv'")
